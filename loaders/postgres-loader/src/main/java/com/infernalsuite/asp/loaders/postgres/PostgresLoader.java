package com.infernalsuite.asp.loaders.postgres;
...
public class PostgresLoader extends UpdatableLoader {
    ...
    private static final String CREATE_VERSIONING_TABLE_QUERY = "CREATE TABLE IF NOT EXISTS database_version (id SERIAL PRIMARY KEY, version INT);";
    private static final String INSERT_VERSION_QUERY = "INSERT INTO database_version (id, version) VALUES (1, ?) ON CONFLICT (id) DO UPDATE SET version = EXCLUDED.version;";
    private static final String GET_VERSION_QUERY = "SELECT version FROM database_version WHERE id = 1;";

    // v1 update query
    private static final String ALTER_LOCKED_COLUMN_QUERY = "ALTER TABLE worlds ALTER COLUMN locked TYPE BIGINT USING locked::BIGINT;";

    // World handling queries
    private static final String CREATE_WORLDS_TABLE_QUERY = "CREATE TABLE IF NOT EXISTS worlds (id SERIAL PRIMARY KEY, name VARCHAR(255) UNIQUE, locked BIGINT, world BYTEA);";
    private static final String SELECT_WORLD_QUERY = "SELECT world FROM worlds WHERE name = ?;";
    private static final String UPDATE_WORLD_QUERY = "INSERT INTO worlds (name, world) VALUES (?, ?) ON CONFLICT (name) DO UPDATE SET world = EXCLUDED.world;";
    ...
    public PostgresLoader(String sqlURL, String host, int port, String database, boolean useSSL, String username, String password) throws SQLException {
